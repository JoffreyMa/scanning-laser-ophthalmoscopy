...
meije_path = osp.join(output_dir, 'star21_OSC_meije.jpg')
meije_low_path = osp.join(output_dir, 'star21_OSC_meije_low.jpg')
meije_high_path = osp.join(output_dir, 'star21_OSC_meije_high.jpg')
meije_hyst_path = osp.join(output_dir, 'star21_OSC_meije_hyst.jpg')
...
#meije = meijering(no_small, sigmas=[5], alpha=0.1, black_ridges=False) 
meije = meijering(no_small, sigmas=[4], alpha=0.7, black_ridges=False) 
skimage.io.imsave(meije_path, meije)
...
low = 75
high = 130
lowt = (meije*255 > low).astype(int)
hight = (meije*255 > high).astype(int)
hyst = apply_hysteresis_threshold(meije*255, low, high)
# Save the hysteresis images
skimage.io.imsave(meije_low_path, lowt)
skimage.io.imsave(meije_high_path, hight)
skimage.io.imsave(meije_hyst_path, hyst)
